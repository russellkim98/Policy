...
%
% Uses an offline knowledge gradient policy with a logistic regression
% belief model to come up with a bid to place in the next auction. Takes in
% a matrix X of alternative actions (discretized bids) and matrices w_est
...
for i=1:M % for each alternative, calculate the KG value
    ...
    KG(i)=sigmoid(kappa(sigma_squared)*mu)*max(profits.*sigmoid(kappa(sigma_x_squared_1).*mu_1))...
        +(1-sigmoid(kappa(sigma_squared)*mu))*max(profits.*sigmoid(kappa(sigma_x_squared_0).*mu_0));
end
[~,maxX]=max(KG);
